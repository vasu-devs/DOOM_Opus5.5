...
| | |
|---|---|
| **Engine** | DDA raycaster, textured walls, perspective floor casting, z-buffered billboard sprites, depth fog, sliding doors |
| **Combat** | 3 weapons (hitscan with per-weapon spread/pellets/cooldown), 3 enemy types with their own AI, dodgeable projectiles, armour damage model |
| **Campaign** | 3 hand-authored sectors, fog-of-war minimap, pickups, score, per-sector stats |
...
| **Quality** | 47 unit/integration tests including a headless run where the agent completes the campaign; zero runtime dependencies |

**Controls** — `WASD` move · mouse aim · click/`Space` fire · `1`–`3` weapons ·
`E` doors · `Tab` map · `P` autoplay · `Esc` pause · `R` restart. Touch controls
appear automatically on coarse-pointer devices.
...
```
runs           : 12
campaigns won  : 12 (100%)
avg sim time   : 70.9s of in-game time per winning run
avg real time  : 1008ms of CPU per run
avg score      : 5624.9
avg accuracy   : 86%
```
...
Walls are cast per column with a DDA and drawn into a 480×300 `ImageData`
buffer, which is then upscaled with smoothing off. The floor pass exploits the
fact that every screen row below the horizon shows floor at a constant distance,
so one divide per row gives a world-space step per pixel — perspective-correct
floors without a per-pixel divide. Sprites are billboarded and clipped against
the wall z-buffer. Cost is O(width × height) per frame, independent of map size.
...
projectile rules, the damage/armour/ammo model, level progression — and a set of
agent tests that run it headlessly and assert it clears the campaign, never
walks through a wall, and never holds a weapon it cannot feed.
